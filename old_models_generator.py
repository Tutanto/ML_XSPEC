...
Description:
    This script is designed for generating and saving synthetic spectral models utilizing the XSPEC astrophysical 
    data analysis package. It employs a Latin Hypercube sampling technique to select parameters for a reflection model,
    followed by scaling these samples to adhere to specific parameter boundaries. The process involves iterating over
    the scaled samples, configuring XSPEC models with these parameters, and storing the resulting models as JSON 
    files. Additionally, it manipulates these JSON files to construct a comprehensive and aggregated DataFrame.
...
Outputs:
    - Synthetic spectral models in JSON format, stored in the 'models' directory.
    - Log files containing execution details, stored in the 'logs' directory.
    - Checkpoint files (last correct index and set of parameters) saved in 'checkpoints' directory.
    - A random sample of generated models is plotted in an array of plots.
    - Grouped and aggregated data from JSON files is stored in a Pandas DataFrame.
...
sample_scaled_file_name = 'sample_scaled.npy'
index_file_name = 'last_successful_index.txt'
...
sample_scaled_file_path = path_to_checkpoints / sample_scaled_file_name
...
# Check if sample_scaled already exists
if (sample_scaled_file_path).is_file():
    logger.debug("Latin Hypercube exists already. Loading from disk.")
    # Load the existing sample_scaled
    sample_scaled = np.load(sample_scaled_file_path)
